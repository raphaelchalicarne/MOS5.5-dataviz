...
#%% Load unique tracks ID list of Portugal
portugal_unique_tracks = np.array([])

t0 = time.time()
csv_array = np.loadtxt(
    'top_portugal_200_spotify.csv', delimiter=",", dtype=object)
portugal_unique_tracks = np.concatenate(
    (portugal_unique_tracks, np.unique(csv_array[1:, 1:])))
unique_tracks_list = np.unique(portugal_unique_tracks)
...
t1 = time.time()
spotify_dict = {}
for portugal_track_id in portugal_unique_tracks:
    if (portugal_track_id not in top_200_af_array):
        track_af = sp.audio_features(portugal_track_id)[0]
        if track_af != None:
            spotify_dict[portugal_track_id] = track_af

            track_data = sp.track(portugal_track_id)
            track_name = track_data['name']
            track_artists = get_artists(track_data)

            spotify_dict[portugal_track_id]['name'] = track_name
            spotify_dict[portugal_track_id]['artists'] = track_artists
            
print("Temps de calcul spotify_dict :",
      "{0:.2f}".format(time.time()-t1), "secondes")

#%% Write missing data in new CSV file
with open('top_200_portugal_af_spotify.csv', mode='w', newline='') as csv_spotify_af:
